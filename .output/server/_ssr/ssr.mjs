...
var ssr_exports = /* @__PURE__ */ __exportAll({
	...
	i: () => createCsrfMiddleware,
	n: () => subdomainPrefixForHost,
	r: () => renderErrorPage,
	t: () => physicalPath
});
require_react();
...
async function getStartManifest(matchedRoutes) {
	const { tsrStartManifest } = await import("../_tanstack-start-manifest_v-C1JXUvMc.mjs");
...
async function loadEntries() {
	const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
		import("./router-TQIET5v8.mjs").then((n) => n.t),
		import("./start-BF0Swg0p.mjs"),
		import("./empty-plugin-adapters-dqO0r5lc.mjs")
	]);
	return {
		routerEntry,
		startEntry,
...
* blog.quantum-basics.com/...           <->  logical route /blog/...  (reserved — no blog routes exist yet)
*/
var SUBDOMAIN_PREFIXES = {
	academy: "/academy",
	blog: "/blog"
};
function subdomainPrefixForHost(hostname) {
...
function physicalPath(hostname, logicalPath) {
...
var PREFIX_TO_SUBDOMAIN_LABEL = Object.fromEntries(Object.entries(SUBDOMAIN_PREFIXES).map(([label, prefix]) => [prefix, label]));
/**
...
*/
function redirectToSubdomain(request) {
	const url = new URL(request.url);
	const hostLabel = url.hostname.split(".")[0];
	if (hostLabel && SUBDOMAIN_PREFIXES[hostLabel]) return null;
	const [, firstSegment, ...rest] = url.pathname.split("/");
	const targetLabel = firstSegment ? PREFIX_TO_SUBDOMAIN_LABEL[`/${firstSegment}`] : void 0;
...
var server_default = createServerEntry({ async fetch(request) {
	try {
		const redirect = redirectToSubdomain(request);
		if (redirect) return redirect;
		return await normalizeCatastrophicSsrResponse(await server_default$1.fetch(request));
	} catch (error) {
		console.error(error);
		return new Response(renderErrorPage(), {
			status: 500,
...
//#endregion
export { ssr_exports as a, renderErrorPage as i, createMiddleware as n, subdomainPrefixForHost as o, physicalPath as r, createCsrfMiddleware as t };
